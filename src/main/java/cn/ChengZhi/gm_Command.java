    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command comand, @NotNull String label, @NotNull String[] args) {
        if (args.length == 1) {
            if (sender instanceof Player) {
                ...
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&',"&c" + args1 + "不是一个有效的模式"));
            }else {
                sender.sendMessage(ChatColor.translateAlternateColorCodes('&',"&c用法错误，用法:/gamemode <模式> [玩家]"));
            }
        }else {
            if (args.length == 2) {
                String args1 = args[0];
