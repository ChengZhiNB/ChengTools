...
import static cn.ChengZhi.main.instance;
        ...
        if (sender instanceof Player) {
            ...
            if (player.isOp()) {
                if (!instance.getConfig().getBoolean(CommandName + "CommandSetting.AllowAdminUsed" + CommandName + "Command")) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&c这个服务器禁止管理员使用heal命令！"));
                    return false;
                }
            }else {
                if (!instance.getConfig().getBoolean(CommandName + "CommandSetting.AllowPlayerUsed" + CommandName + "Command")) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&c这个服务器禁止玩家使用heal命令！"));
                    return false;
                }
            }
